...
def main(args):
    ...
    train, test = train_test_split(
        df, test_size=args.train_test_split_ratio, random_state=args.random_state
    )

    logger.info(train.dtypes)

    numerical_idx = (
        train.select_dtypes(exclude=["object", "category"])
        .drop(["event", "duration"], 1)
        .columns.tolist()
    )

    categorical_idx = train.select_dtypes(exclude=["float", "int"]).columns.tolist()
